...
class SyncVerifiedUserTiers extends Command
{
    ...
    public function handle()
    {
        ...
        User::whereNotNull('email_verified_at')->chunk(100, function ($users) use (&$updatedCount) {
            foreach ($users as $user) {
                ...
                $isIdentityVerified = DB::table('kyc_verifications')
                ...
                $hasSelfie = DB::table('kyc_verifications')
                    ->where('user_id', $user->id)
                    ->where('status', 'verified')
