      ...
      <form @submit.prevent="submit">
        ...
        <div v-if="currentStep === 1" class="space-y-4 animate-fadeIn">
          <div>
            ...
            <input v-model="name" type="text" class="w-full px-4 py-2 bg-transparent border border-gray-600 rounded-lg focus:border-[#00D4FF] outline-none" required />
          ...
          <div>
            ...
            <input v-model="email" type="email" class="w-full px-4 py-2 bg-transparent border border-gray-600 rounded-lg focus:border-[#00D4FF] outline-none" required />
          </div>

          <div>
            <label class="block mb-1 text-gray-300">Password</label>
            <input v-model="password" type="password" class="w-full px-4 py-2 bg-transparent border border-gray-600 rounded-lg focus:border-[#00D4FF] outline-none" required />
            <div class="flex gap-1 mt-2">
              <div v-for="i in 3" :key="i" :class="['h-1 w-full rounded-full', password.length >= (i*3) ? 'bg-green-' : 'bg-gray-700']"></div>
            </div>
            <p class="mt-1 text-xs text-gray-500">Strength: {{ password.length < 5 ? 'Weak' : 'Strong' }}</p>
          </div>

          <div>
            <label class="block mb-1 text-gray-300">Confirm Password</label>
            <input v-model="password_confirmation" type="password" class="w-full px-4 py-2 bg-transparent border border-gray-600 rounded-lg focus:border-[#00D4FF] outline-none" required />
            <p v-if="password !== password_confirmation && password_confirmation" class="mt-1 text-xs text-red-400">Passwords do not match</p>
          </div>

          <button type="button" @click="nextStep" class="w-full bg-[#00D4FF] text-[#0B132B] py-2 rounded-lg font-bold hover:opacity-90">
            Continue
          </button>
        </div>
        ...
        <div v-if="currentStep === 2" class="space-y-4 animate-fadeIn">
          <div class="p-3 mb-4 border rounded-lg bg-blue-500/10 border-blue-500/30">
            <p class="text-xs leading-tight text-blue-300">
              ⓘ Adding these now speeds up your verification and enables instant transactions. You can skip this for now.
            </p>
          </div>

          <div>
            <label class="block mb-1 text-gray-300">BVN (Optional)</label>
            <input v-model="kyc.bvn" type="text" maxlength="11" placeholder="11-digit BVN" class="w-full px-4 py-2 bg-transparent border border-gray-600 rounded-lg focus:border-[#00D4FF] outline-none" />
          </div>

          <div>
            <label class="block mb-1 text-gray-300">NIN (Optional)</label>
            <input v-model="kyc.nin" type="text" maxlength="11" placeholder="11-digit NIN" class="w-full px-4 py-2 bg-transparent border border-gray-600 rounded-lg focus:border-[#00D4FF] outline-none" />
          </div>

          <div class="flex gap-3 pt-2">
            <button type="button" @click="currentStep--" class="w-1/3 py-2 font-semibold border border-gray-600 rounded-lg">Back</button>
        ...
        <div v-if="currentStep === 3" class="space-y-4 animate-fadeIn">
          <div class="bg-[#151a27] p-4 rounded-xl border border-gray-700">
            ...
            <p class="text-sm">Verification: <span class="text-white">{{ kyc.bvn || kyc.nin ? 'Provided' : 'Pending' }}</span></p>
          </div>
          ...
          <div class="flex gap-3">
            <button type="button" :disabled="loading" @click="currentStep--" class="w-1/3 py-2 font-semibold border border-gray-600 rounded-lg disabled:opacity-50">Back</button>
