    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex gap-3">
          <div class="relative">
            <input ref="searchInputRef" v-model="search" @input="handleSearchInput"
              @keydown.enter="selectFirstSuggestion" type="text" placeholder="Search global stocks..."
            ...
            <span v-if="searchLoading"
              class="absolute text-xs text-gray-400 -translate-y-1/2 right-3 top-1/2">Searching…</span>

            <!-- Autocomplete Dropdown -->
            <div v-if="searchSuggestions.length > 0"
              class="absolute top-full left-0 right-0 mt-1 bg-[#0F1724] border border-[#1f3348] rounded-lg shadow-2xl z-50 max-h-48 overflow-y-auto">
              <div v-for="suggestion in searchSuggestions" :key="suggestion.symbol"
      ...
      <div class="flex flex-col gap-4 mb-6 md:flex-row md:items-center">
        <div>
          <p class="text-[12px] uppercase tracking-widest text-white font-bold mb-1">USD Wallet Balance:
            <span> {{ (
              walletBalances.cleared_balance_usd.toLocaleString(undefined, { minimumFractionDigits: 2 })) }}
            </span>
          </p>
        </div>
        <div>
          <p class="text-[12px] uppercase tracking-widest text-white font-bold mb-1">Global Stocks Value:
            <span> ${{ (
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          <div class="flex p-1 bg-[#0B121D] border border-[#1f3348] rounded-lg">
            <button @click="activeChart = 'holdings'"
          ...
          <div v-if="activeChart === 'market'" class="flex flex-wrap items-center gap-2">
            ...
            <div class="relative" ref="chartSearchContainer">
              ...
              <span v-if="chartSearchLoading"
                class="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-gray-500 italic">Searching...</span>
              <div v-if="chartSearchResults.length > 0"
                class="absolute top-full left-0 right-0 mt-1 bg-[#0F1724] border border-[#1f3348] rounded-lg shadow-2xl z-50 max-h-48 overflow-y-auto">
                <div v-for="res in chartSearchResults" :key="res.symbol" @click="selectForChart(res)"
                  class="p-2 hover:bg-[#16213A] cursor-pointer border-b border-[#1f3348]/50 last:border-0 text-xs">
        ...
        <div class="mt-4">
          <div v-if="activeChart === 'insights'">
            <MarketInsights :insightData="globalApiInsights" :loading="isInsightsLoading" currencySymbol="$"
              @trade="openTrade" />
          </div>
          <div v-else-if="activeChart === 'holdings'">
            <HoldingPerformanceChart title="My Global Stocks Holdings" currencySymbol="$" :seriesData="portfolioData"
              ...
              <div class="overflow-x-auto">
                <div v-if="showSearchResults" class="p-4">
                  ...
                  <div class="divide-y divide-[#1f3348]">
                    <div v-for="stock in searchResults" :key="stock.symbol"
          ...
          <div v-else class="space-y-4">
            <div class="flex items-center gap-3">
              <span
                class="text-[10px] text-gray-500 uppercase font-black tracking-widest whitespace-nowrap bg-[#16213A] px-2 py-1 rounded border border-[#1f3348]">My
                favorite tickers</span>
              <MarketTicker class="flex-1" :selected-symbol="selectedMarketSymbol" :additional-tickers="searchedTickers"
