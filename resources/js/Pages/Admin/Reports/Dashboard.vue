  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-400">Today's Summary - {{ new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) }}</p>
      </div>
    </div>

    <!-- Today's Summary Cards -->
    <div class="mb-8">
      <h2 class="mb-4 text-lg font-semibold text-white">Today's Summary</h2>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <div class="bg-[#0F1724] border border-[#1f3348] rounded-xl p-5">
        <h3 class="mb-4 text-sm font-medium text-gray-300">Users Registered (Last 30 Days)</h3>
        <apexchart v-if="charts?.usersGrowth" type="area" height="300" :options="chartOptions('#0047AB')" :series="charts.usersGrowth.series" />
      </div>
      <div class="bg-[#0F1724] border border-[#1f3348] rounded-xl p-5">
        <h3 class="mb-4 text-sm font-medium text-gray-300">Deposits vs Withdrawals</h3>
      ...
      <div class="bg-[#0F1724] border border-[#1f3348] rounded-xl p-5">
        <h3 class="mb-4 text-sm font-medium text-gray-300">Investments</h3>
        <apexchart v-if="charts?.investments" type="bar" height="300" :options="chartOptions('#8B5CF6', 'bar')" :series="charts.investments.series" />
      </div>
      <div class="bg-[#0F1724] border border-[#1f3348] rounded-xl p-5">
        <h3 class="mb-4 text-sm font-medium text-gray-300">Revenue</h3>
