<template>
  <div class="space-y-6">
    <div class="flex items-center justify-end">
      <button type="button" @click="$router.push('/admin/expenses/create')" class="px-4 py-2 bg-[#0047AB] text-white rounded-lg text-sm">+ Create Expense</button>
    </div>

    <!-- Summary -->
    <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
      <div class="bg-[#0F1724] border border-[#1f3348] rounded-xl p-4">
        <p class="mb-1 text-xs tracking-wider text-gray-400 uppercase">Total Expenses</p>
        <p class="font-mono text-lg font-bold text-white">{{ totalCard.main }}</p>
        <p v-if="totalCard.extras.length" class="text-[11px] text-gray-500 mt-0.5 truncate">{{ totalCard.extras.join(' · ') }}</p>
      </div>
      <div class="bg-[#0F1724] border border-[#1f3348] rounded-xl p-4">
        <p class="mb-1 text-xs tracking-wider text-gray-400 uppercase">Draft</p>
        <p class="font-mono text-lg font-bold text-gray-300">{{ draftCard.main }}</p>
        <p v-if="draftCard.extras.length" class="text-[11px] text-gray-500 mt-0.5 truncate">{{ draftCard.extras.join(' · ') }}</p>
      </div>
      <div class="bg-[#0F1724] border border-[#1f3348] rounded-xl p-4">
        <p class="mb-1 text-xs tracking-wider text-gray-400 uppercase">Approved</p>
        <p class="font-mono text-lg font-bold text-blue-400">{{ approvedCard.main }}</p>
        <p v-if="approvedCard.extras.length" class="text-[11px] text-gray-500 mt-0.5 truncate">{{ approvedCard.extras.join(' · ') }}</p>
      </div>
      <div class="bg-[#0F1724] border border-[#1f3348] rounded-xl p-4">
        <p class="mb-1 text-xs tracking-wider text-gray-400 uppercase">Paid</p>
        <p class="font-mono text-lg font-bold text-green-400">{{ paidCard.main }}</p>
        <p v-if="paidCard.extras.length" class="text-[11px] text-gray-500 mt-0.5 truncate">{{ paidCard.extras.join(' · ') }}</p>
      </div>
    </div>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="expense in expenses.data" :key="expense.id" class="border-b border-[#1f3348] hover:bg-[#16213A]">
            ...
            <td class="px-4 py-3 font-mono text-right text-white">{{ formatNumber(expense.amount) }}</td>
...
// Amount figures across the other admin pages use symbols (₦, $…) rather than
// ISO codes, so summary cards match. Unknown codes fall back to their ISO form.
...
const currencySymbol = (code) => (
  CURRENCY_SYMBOLS[code] !== undefined ? CURRENCY_SYMBOLS[code] : `${code} `
);

// Multi-currency safe card rendering: the highest-value currency is shown as
// the main figure, any additional currencies appear as a secondary line so
// amounts are never silently merged across currencies.
const buildSummaryCard = (breakdown) => {
  const entries = Object.entries(breakdown || {})
    .filter(([, value]) => Number(value) > 0)
    .sort(([, a], [, b]) => b - a);

  if (!entries.length) {
    return { main: `${currencySymbol(filters.value.currency || 'NGN')}0.00`, extras: [] };
  }

  return {
    main: `${currencySymbol(entries[0][0])}${formatNumber(entries[0][1])}`,
    extras: entries.slice(1).map(([currency, amount]) => `+${currencySymbol(currency)}${formatNumber(amount)}`),
  };
};

const totalCard = computed(() => buildSummaryCard(summary.value.total_by_currency));
const draftCard = computed(() => buildSummaryCard(summary.value.draft_by_currency));
const approvedCard = computed(() => buildSummaryCard(summary.value.approved_by_currency));
const paidCard = computed(() => buildSummaryCard(summary.value.paid_by_currency));
