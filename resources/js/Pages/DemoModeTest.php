...
class DemoModeTest extends TestCase
{
    ...
    public function test_user_in_demo_mode_can_fund_wallet_instantly_via_deposit_endpoint(): void
    {
        ...
        $initialBalance = DemoWallet::where('user_id', $this->user->id)->value('balance_ngn');
        $fundingAmount = 50000;
        ...
        $newBalance = DemoWallet::where('user_id', $this->user->id)->value('balance_ngn');
    ...
    public function test_user_in_demo_mode_can_place_buy_trade(): void
    {
        ...
        $wallet = DemoWallet::where('user_id', $this->user->id)->first();
        $initialBalance = $wallet->balance_ngn;
        $tradeAmount = 10000;
        $marketPrice = 50.00;
        ...
        $wallet->refresh();
        $this->assertEquals($initialBalance - $tradeAmount, $wallet->balance_ngn);
    ...
    public function test_user_can_reset_demo_account(): void
    {
        ...
        $wallet = DemoWallet::where('user_id', $this->user->id)->first();
        $wallet->update(['balance_ngn' => 50000]);
        ...
        $wallet->refresh();
        $this->assertEquals(1000000, $wallet->balance_ngn);
    }
}
