  ...
  <MainLayout>
    <div class="mx-auto max-w-2xl space-y-6">
      <div class="rounded-xl border border-emerald-500/30 bg-[#0F1724] p-8 text-center">
        ...
        <SkeletonLoader v-if="!investment && !error" type="card" :count="1" class="mt-8 opacity-40" />
        ...
        <div v-if="investment" class="mt-8 grid gap-4 text-left sm:grid-cols-2">
          ...
          <div>
            ...
            <p class="mt-1 text-white">{{ fixedIncomeCurrency(investment.principal_amount, investment.currency) }}</p>
          </div>
          <div>
            <p class="text-sm text-gray-500">Expected interest</p>
            <p class="mt-1 text-white">{{ fixedIncomeCurrency(investment.expected_interest, investment.currency) }}</p>
          </div>
          <div>
            <p class="text-sm text-gray-500">Expected maturity</p>
            <p class="mt-1 text-white">{{ fixedIncomeCurrency(investment.expected_maturity_amount, investment.currency) }}</p>
          </div>
          <div>
            <p class="text-sm text-gray-500">Status</p>
            <p class="mt-1 text-cyan-400">{{ fixedIncomeLabel(investment.status) }}</p>
          </div>
        </div>
