...
Schedule::call(function () {
    User::where('subscription_status', 'trial')
        ...
        ->each(fn ($user) => $user->notify(new TrialEndingNotification(2)));
})->dailyAt('09:00');

// Fee due reminders (3 days before next charge)
Schedule::call(function () {
    User::whereIn('subscription_status', ['trial', 'active'])
        ->whereBetween('next_fee_due_at', [now()->addDays(3)->startOfDay(), now()->addDays(3)->endOfDay()])
        ->each(fn ($user) => $user->notify(new BillingAlertNotification(1000, 'Platform fee due in 3 days')));
})->dailyAt('09:00');
...
Schedule::call(function () {
    ...
    User::where('next_fee_due_at', '<=', now())
